...
import CartItems from '../../common/cartItems.tsx/Index'

// NOTE: this is a child component of main cartPopup index
function ZeroItems() {
...
// this is child component of main cartPopup indexNOTE:
function CartItemsList({ cartItems }: any) {
    return (
...
function CartPopUp() {
    const { openCartSidebar, cartItems, cartTotalQuantity } = useSelector(getCart)
    const dispatch = useDispatch()
    const router = useRouter()

    // if cart is open then add this styles to body tag-----------
    useEffect(() => {
        if (openCartSidebar) {
                ...
                {cartItems.length >= 1 && (
                    <div className="w-full flex justify-center items-center">
                        <button
                            ...
                            onClick={() => (
                                router.push('/shop/cart'), dispatch(openCart())
                            )}
                        >
                            Go to Cart
                        </button>
